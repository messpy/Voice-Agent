...
- `ベリベリ音量上げて` のような inline command 起動
- 統合ランナーでの運用モード切替
...
wake:
  ...
  porcupine:
    ...
    keyword_paths:
      ベリベリ: /path/to/beriberi_ja_raspberry-pi.ppn
...
`ベリベリ` と `ずんだもん` は専用 `.ppn` を追加で置く必要があります。
...
`music.play` は `voicechat-actions` 側で `yt-dlp` による再生前 probe を行い、取れたタイトルを `message` に返します。
