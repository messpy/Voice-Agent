...
このリポジトリ単体ではなく、次の sibling 構成を前提にしています。
...
mkdir -p ~/work
cd ~/work
git clone <voicechat-url> voicechat
git clone <voicechat-actions-url> voicechat-actions
git clone <natureremo-url> natureremo
...
`voicechat/config/config.yaml` の `command_router.action_runners.external_cli.cwd` は `../voicechat-actions` を前提にしています。  
`voicechat-actions/config/config.yaml` の Nature Remo 送信は sibling の `natureremo` を探します。
...
依存サービスもまとめて扱いたい場合は、ワークスペース共通の [systemd/README.md](/home/kennypi/work/systemd/README.md) にある `voicechat.target` を使います。
この target は `shared-voicevox.service` と `shared-ollama.service` を一緒に起動します。
...
ブラウザ管理コンソールを起動する場合は次を使います。

```bash
cd <repo-root>
./voicechat.sh web-console --host 127.0.0.1 --port 8787
```

他端末から見る場合は `--host 0.0.0.0` で起動し、`8787/tcp` を firewall で許可してください。

機能:

- 現在状態の表示
- recent command / wake event の閲覧
- `config/config.yaml` の表示と保存
- ブラウザからの command 送信

`internal_*` command のうち、認識モード変更と AI 切替は設定ファイルへ保存する形で扱います。
実行中の `voicechat` プロセスへは即時反映されないため、必要なら再起動してください。
...
`search_query` を渡すと `ytsearch5:` で検索して先頭候補の動画を 1 件再生できます。
`music.next` を command に追加すると、再生中の曲を止めて次の候補へ進められます。
検索再生の入力例は `YouTubeでディズニーを再生して` や `ディズニーの動画を再生して` です。
